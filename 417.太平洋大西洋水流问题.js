...
var pacificAtlantic = function (heights) {
    ...
    let res = [], memo = {}
    const helper = (i, j, visited, cb) => {
        let key = `${i}_${j}_${cb.name}`
        visited.add(`${i}_${j}`)
        ...
        for (const [x, y] of DIRS) {
            let nextI = i + x
            let nextJ = j + y
            if (!visited.has(`${nextI}_${nextJ}`)) {
                if (nextI >= 0 && nextI < m && nextJ >= 0 && nextJ < n) {
                    if (heights[nextI][nextJ] <= curr) {
                        if (helper(nextI, nextJ, visited, cb)) {
                            memo[key] = true
                            return true
                        }
                    }
    ...
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
            const res1 = helper(i, j, new Set(), function a(i, j) {
                if (i === 0 || j === 0) { return true }
            })
            memo[`${i}_${j}_a`] = res1
            const res2 = helper(i, j, new Set(), function b(i, j) {
                if (i === m - 1 || j === n - 1) { return true }
            })
            memo[`${i}_${j}_b`] = res2
            if (res1 && res2) { res.push([i, j]) }
        }
    }
    return res
};
...
console.timeEnd(1)
console.log('res :>> ', res);
var res2 = [[0, 4], [1, 3], [1, 4], [2, 2], [3, 0], [3, 1], [4, 0]]