...
class SmallestInfiniteSet {
    constructor() {
        this.arrSet = new Set(new Array(1000).fill(0).map((_, idx) => _ = idx + 1))
    }
    /**
     * @return {number}
     */
    popSmallest() {
        let idx = 1
        while (!this.arrSet.has(idx)) {
            idx++
        }
        this.arrSet.delete(idx)
        return idx
    }
    /**
     * @param {number} num
 ...
 * obj.addBack(num)
 */
["", "pop", "addBack", "pop", "pop", "pop", "addBack", "addBack", "pop", "pop"]
[[], [], [1], [], [], [], [2], [3], [], []]
// [null,1,null,1,2,3,null,null,3,2]
// [null,1,null,1,2,3,null,null,2,3]

var smallestInfiniteSet = new SmallestInfiniteSet();
// console.log(smallestInfiniteSet.addBack(2));    // 2 已经在集合中，所以不做任何变更。
console.log(smallestInfiniteSet.popSmallest()); // 返回 1 ，因为 1 是最小的整数，并将其从集合中移除。
console.log(smallestInfiniteSet.addBack(1));    // 将 1 添加到该集合中。
...
console.log(smallestInfiniteSet.addBack(2));    // 将 1 添加到该集合中。
console.log(smallestInfiniteSet.addBack(3));    // 将 1 添加到该集合中。
