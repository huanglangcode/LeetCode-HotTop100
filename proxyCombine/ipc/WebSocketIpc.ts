...
export class WebSocketIpc extends BaseIpc {
  ...
  public send = (channel: string, ...args: any[]) => {
    if (
      ...
      this.instance.send === undefined ||
      this.instance.send === null
    ) {
      log.error(
        "WebSocketIpc instance don't have send method. instance=",
        this.instance
      );
      return;
    }
  ...
  public sendSync = (channel: string, ...args: any[]): Promise<any> | any => {
    if (
      ...
      log.error(
        "WebSocketIpc instance don't have sendSync method. instance=",
        this.instance
      );
      return;
    }
  ...
  public on = (channel: string, listener: (...args) => void) => {
    this.instance.addEventListener("message", (event) => {
      ...
      this.instance.emit(
        params[0],
        ...[{ sender: this.instance }, params[1], params[2]]
      );
    });
  ...
  public removeAllListeners = (channel: string) => {
    if (
      ...
      log.error(
        "WebSocketIpc instance don't have removeAllListeners  method. instance=",
        this.instance
      );
      return;
    }
  ...
  public removeListener = (channel: string, listener: (e, ...args) => void) => {
    if (
      ...
      log.error(
        "WebSocketIpc instance don't have removeListener method. instance=",
        this.instance
      );
      return;
    }
