...
export type Request = UnknownRequest | GetRequest | ApplyRequest | SyncRequest | ReplyRequest
    ...
    Object.getOwnPropertyNames(Object.getPrototypeOf(target)).forEach(propKey => {
        if (propKey !== 'constructor') {
            ...
            Object.defineProperty(result, propKey, {
                enumerable: true,
                get: memoize(() => getProperty(functionType, propKey, target.channel, transport))
            });
        }
    });
...
function getProperty(propertyType: ProxyPropertyType, propKey: string, channel: string, transport) {
    switch (propertyType) {
        ...
        case ProxyPropertyType.FunctionRely:
            return (...args: any[]) => makeRequest({ type: RequestType.Reply, propKey, args }, channel, transport);
        default:
...
function makeRequest(request: Request, channel: string, transport: BaseIpc): Promise<any> {
...
class ProxyServerHandler {
    ...
    private handleReply(request: ApplyRequest | SyncRequest | ReplyRequest, sender: any): any {
        const { propKey, args } = request;
        const func = this.target[propKey];
